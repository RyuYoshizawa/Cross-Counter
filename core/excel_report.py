...
def _write_cross_split_block(ws: Worksheet, row: int, r: dict) -> int:
    ...
    _write_index_and_pct_labels(ws, row, all_cols, first_col, total_col, index_label='%', total_label=_TOTAL_LABEL)
    row += 2
    header_row = row
    ...
    row += 1
    row = _write_target_group_header(ws, row, target_label, n_opts, first_col)
    _write_index_and_pct_labels(ws, row, all_cols, first_col, total_col, index_label='実数', total_label=_TOTAL_LABEL)
    row += 2
    header_row = row
    for cat in [*attr_rows, _TOTAL_LABEL]:
        is_total = cat == _TOTAL_LABEL
...
def _write_cross_split_old_block(ws: Worksheet, row: int, r: dict) -> int:
    ...
    for i, _ in enumerate(all_cols, 1):
